import type { StateAdapter } from '@pixel-agents/core/adapter.js';
import type { ServerMessage } from '@pixel-agents/core/messages.js';
import { AgentRuntime } from '@pixel-agents/server/agentRuntime.js';
import { AgentStateStore } from '@pixel-agents/server/agentStateStore.js';
import type { LoadedAssets, LoadedCharacterSprites } from '@pixel-agents/server/assetLoader.js';
import {
...
} from '@pixel-agents/server/assetLoader.js';
import { readConfig, writeConfig } from '@pixel-agents/server/configPersistence.js';
import { setTerminalAdapter } from '@pixel-agents/server/fileWatcher.js';
import { InProcessBridge } from '@pixel-agents/server/inProcessBridge.js';
import type { LayoutWatcher } from '@pixel-agents/server/layoutPersistence.js';
import {
  readLayoutFromFile,
  watchLayoutFile,
  writeLayoutToFile,
} from '@pixel-agents/server/layoutPersistence.js';
import { claudeProvider, copyHookScript } from '@pixel-agents/server/providers/index.js';
import { PixelAgentsServer } from '@pixel-agents/server/server.js';
...
export class PixelAgentsViewProvider implements vscode.WebviewViewProvider {
  ...
  // In-process bridge: routes messages between webview postMessage and server runtime
  // without a real network hop. Future: replace deep server imports with bridge-only comms.
  readonly bridge = new InProcessBridge();
  ...
  constructor(
    ...
    this.store.on('agentRemoved', (id) => {
      this.bridge.send({ type: 'agentClosed', id });
    });
    this.store.on('broadcast', (message) => {
      this.bridge.send(message as unknown as ServerMessage);
    });

    // Bridge → webview: forward every server message as a postMessage
